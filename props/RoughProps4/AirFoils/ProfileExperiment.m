...
%% transformations 
chord = max(normalized(:,1))-min(normalized(:,1));  % chord length
shifted = normalized;
...
% now making the transformations
    ...
    first_Profile = shifted * first_Chord; % scaling
    first_Profile(:,1) = first_Profile(:,1)+x_Upper(first_y); % moving the airfoil to 1/4 chord
    ...
    last_Profile(:,1) = last_Profile(:,1)+x_Upper(last_y); 
...
second_Test = [x_Upper(1),x_Lower(1);0,0];
plot(second_Test(1,:), second_Test(2,:) );

third_Test = rotate2D(second_Test,50); 
plot(third_Test(1,:), third_Test(2,:));
