...
offset = 40; % offset for the elipse (so that we get growing, then shrinking) 
x_Upper  = ((60^2 - (y-offset).^2)./10).^(1/2); % 6^2 = x^2+10y^2
x_Lower = ((60^2 - (y-offset).^2)./30).^(1/2)* -1; % 6^2 = x^2+30y^21/4 
...
cd 'P:\University\DuringUni\SubC\Fluids\props\RoughProps4\AirFoils\ReorganizedCode\generated' 
...
initial_Length == transformed_Length;
